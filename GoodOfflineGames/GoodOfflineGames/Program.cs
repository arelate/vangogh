...
namespace GOG
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var gamesDetailsController = new GameDetailsController(gamesDetails);
            ...
            //        if (gogData != null) {
                           //productsDataController.Add(gogData.ProductData);
            ...
            var allowedLanguages = new List<string>() { "English" };
            ...
            foreach (var p in products)
            {
                ...
                {
                    var existingGameDetails = gamesDetailsController.Find(p.Id);
                ...
                if (string.IsNullOrEmpty(gameDetailsString)) continue;

                var gameDetails = JsonConvert.DeserializeObject<GameDetails>(gameDetailsString);
                gameDetails.LanguageDownloads = new List<OperatingSystemsDownloads>();

                if (gameDetails == null) continue;

                foreach (var entry in gameDetails.DownloadsDynamic)
                {
                    if (entry.Length != 2)
                    {
                    ...
                    var downloads = jsonStringController.Deserialize<OperatingSystemsDownloads>(downloadsString);
                    downloads.Language = language;

                    gameDetails.LanguageDownloads.Add(downloads);
                }

                gameDetails.Id = p.Id;
                gameDetails.DownloadsDynamic = null;

                gamesDetailsController.Add(gameDetails);
