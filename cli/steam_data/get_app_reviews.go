...
func GetAppReviews(steamGogIds map[string][]string, force bool) error {
	...
	return ReduceAppReviews(maps.Keys(steamGogIds), kvAppReviews)
}

func ReduceAppReviews(steamAppIds iter.Seq[string], kvAppReviews kevlar.KeyValues) error {
	...
	for steamAppId := range steamAppIds {
		...
		if err = reduceAppReviewsProduct(steamAppId, kvAppReviews, appReviewsReductions); err != nil {
			return err
		}
...
func reduceAppReviewsProduct(steamAppId string, kvAppReviews kevlar.KeyValues, piv shared_data.PropertyIdValues) error {
